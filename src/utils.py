...
async def fetch_message_list_opt(channel_id, msg_ids, purge=False):
    ...
    if None in l:
        if purge:
            await purge_opt_message_list(l)
        return None
    return l
...
async def publish_long_message(messages, channel_id, what):
    ...
    if not what['content'].strip():
        what['content'] = 'empty message'
    ...
    try:
        ...
        if len(what['content']) > MAX_MESSAGE_ALLOWED:
            ...
            good_contents, n_messages = [what['content']], 1
        ...
        for i in range(1, n_messages):
            ...
            await msg.edit(content=last_message(prec.jump_url, n_messages, i+1)
                           + good_contents[i]
                           + (next_message(succ.jump_url, n_messages, i+1)
        ...
        for m in messages:
            try:
                await message.edit("Erreur de publication de message :(", ephemeral=True)
            except Exception:
                pass
        return ';'.join(messages)
